...
if ( ! function_exists( 'the_pipes_customizer_get_css' ) ) {

	function the_pipes_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = the_pipes_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.the_pipes_tabs .the_pipes_tabs_titles li a {
...
CSS;
			$rez          = apply_filters( 'the_pipes_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = the_pipes_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'the_pipes_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( the_pipes_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = the_pipes_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'the_pipes_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'the_pipes_filter_prepare_css', $css_str, $remove_spaces );
	}
}
