...
if ( ! function_exists( 'the_pipes_storage_get' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_get( $var_name, $default = '' ) {
		global $THE_PIPES_STORAGE;
		return isset( $THE_PIPES_STORAGE[ $var_name ] ) ? $THE_PIPES_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'the_pipes_storage_set' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_set( $var_name, $value ) {
		global $THE_PIPES_STORAGE;
		$THE_PIPES_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'the_pipes_storage_empty' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $THE_PIPES_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $THE_PIPES_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $THE_PIPES_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_isset' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $THE_PIPES_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $THE_PIPES_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $THE_PIPES_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_unset' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $THE_PIPES_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $THE_PIPES_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
				unset( $THE_PIPES_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_inc' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_inc( $var_name, $value = 1 ) {
		global $THE_PIPES_STORAGE;
		if ( empty( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = 0;
		}
		$THE_PIPES_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'the_pipes_storage_concat' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_concat( $var_name, $value ) {
		global $THE_PIPES_STORAGE;
		if ( empty( $THE_PIPES_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'the_pipes_storage_get_array' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $THE_PIPES_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ? $THE_PIPES_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_set_array' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_set_array( $var_name, $key, $value ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$THE_PIPES_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'the_pipes_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function the_pipes_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$THE_PIPES_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$THE_PIPES_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_merge_array( $var_name, $key, $value ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$THE_PIPES_STORAGE[ $var_name ] = array_merge( $THE_PIPES_STORAGE[ $var_name ], $value );
		} else {
			$THE_PIPES_STORAGE[ $var_name ][ $key ] = array_merge( $THE_PIPES_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			the_pipes_array_insert_after( $THE_PIPES_STORAGE[ $var_name ], $after, $key );
		} else {
			the_pipes_array_insert_after( $THE_PIPES_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function the_pipes_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			the_pipes_array_insert_before( $THE_PIPES_STORAGE[ $var_name ], $before, $key );
		} else {
			the_pipes_array_insert_before( $THE_PIPES_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_push_array' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_push_array( $var_name, $key, $value ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $THE_PIPES_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ) {
				$THE_PIPES_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $THE_PIPES_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $THE_PIPES_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $THE_PIPES_STORAGE[ $var_name ] ) && is_array( $THE_PIPES_STORAGE[ $var_name ] ) && count( $THE_PIPES_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $THE_PIPES_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) && is_array( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) && count( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $THE_PIPES_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'the_pipes_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function the_pipes_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( empty( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ][ $key ] = 0;
		}
		$THE_PIPES_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'the_pipes_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function the_pipes_storage_concat_array( $var_name, $key, $value ) {
		global $THE_PIPES_STORAGE;
		if ( ! isset( $THE_PIPES_STORAGE[ $var_name ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ] = array();
		}
		if ( empty( $THE_PIPES_STORAGE[ $var_name ][ $key ] ) ) {
			$THE_PIPES_STORAGE[ $var_name ][ $key ] = '';
		}
		$THE_PIPES_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'the_pipes_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $THE_PIPES_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $THE_PIPES_STORAGE[ $var_name ] ) ? $THE_PIPES_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $THE_PIPES_STORAGE[ $var_name ] ) ? $THE_PIPES_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'the_pipes_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function the_pipes_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $THE_PIPES_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $THE_PIPES_STORAGE[ $var_name ]->$prop ) ? $THE_PIPES_STORAGE[ $var_name ]->$prop : $default;
	}
}
