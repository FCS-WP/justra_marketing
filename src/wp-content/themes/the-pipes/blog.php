...
if ( function_exists( 'the_pipes_elementor_is_preview' ) && the_pipes_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			the_pipes_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	the_pipes_new_main_query(
		array(
			'post_type'      => the_pipes_get_theme_option( 'post_type' ),
			'category'       => the_pipes_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'the_pipes_filter_get_template_part', the_pipes_blog_archive_get_template() ) );
}
