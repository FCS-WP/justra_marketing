<div class="front_page_section front_page_section_testimonials<?php
	$the_pipes_scheme = the_pipes_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $the_pipes_scheme ) && ! the_pipes_is_inherit( $the_pipes_scheme ) ) {
		echo ' scheme_' . esc_attr( $the_pipes_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( the_pipes_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( the_pipes_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$the_pipes_css      = '';
		$the_pipes_bg_image = the_pipes_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$the_pipes_anchor_icon = the_pipes_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$the_pipes_anchor_text = the_pipes_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( the_pipes_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$the_pipes_css      = '';
			$the_pipes_bg_mask  = the_pipes_get_theme_option( 'front_page_testimonials_bg_mask' );
			$the_pipes_bg_color_type = the_pipes_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $the_pipes_bg_color_type ) {
				$the_pipes_bg_color = the_pipes_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$the_pipes_caption = the_pipes_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $the_pipes_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $the_pipes_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $the_pipes_caption, 'the_pipes_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$the_pipes_description = the_pipes_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $the_pipes_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $the_pipes_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $the_pipes_description ), 'the_pipes_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! the_pipes_exists_trx_addons() ) {
						...
						the_pipes_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
