...
jQuery( document ).ready(
	function() {
		...
		the_pipes_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="the_pipes_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.the_pipes_post_editor' );
						if ( link.length > 0 ) {
							the_pipes_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.the_pipes_post_editor', function(e) {
						the_pipes_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('the_pipes_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function the_pipes_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
