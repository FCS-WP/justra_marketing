...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			the_pipes_create_observer( 'the-pipes-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				the_pipes_gutenberg_editor_init();
			} );
		} else {
		 	the_pipes_gutenberg_editor_init();
		}
	...
	function the_pipes_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'the_pipes_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'the_pipes_inited' ) ) {
	...
	// Init on page load
	function the_pipes_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = the_pipes_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + THE_PIPES_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + THE_PIPES_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in THE_PIPES_STORAGE['override_classes'] ) {
				$obj.addClass( THE_PIPES_STORAGE['override_classes'][i].replace( '%s', THE_PIPES_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( the_pipes_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && THE_PIPES_STORAGE['sidebar_position'] != 'hide' ) {
				if ( the_pipes_apply_filters( 'the_pipes_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'the_pipes_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( the_pipes_apply_filters( 'the_pipes_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				the_pipes_remove_observer( 'the-pipes-check-editor-styles-wrapper' );
				the_pipes_create_observer( 'the-pipes-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'the_pipes_inited' );
	}
} );
